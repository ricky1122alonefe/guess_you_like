...
from market_control import LEVEL_CN, RISK_CN, analyze_control
from match import SCORE_POOL_TOP_N
from odds_signals import MarketSignals
from trap_signals import TrapAnalysis, analyze_traps, apply_penalties
...
def print_ai_recommendation(data: dict) -> None:
    ...
    if actuary or data.get("implied_probability"):
        ...
        rec = data.get("recommendation") or data.get("result_1x2_cn")
        conf = data.get("confidence_level") or data.get("confidence_cn")
        if rec:
            print(f"    精算建议：{rec}（置信 {conf or '—'}）")
        if actuary:
