...
ODDS_BUCKET_EDGES = {
    # 主胜赔 / 热门最短赔 共用桶
    "home": [1.01, 1.30, 1.45, 1.60, 1.85, 2.20, 2.80, 4.00],
    # 平赔桶
    "draw": [1.01, 2.60, 2.90, 3.10, 3.30, 3.60, 4.00],
}
ODDS_BUCKET_AH_RANGES = [
    # (label, low, high]；None 表示无穷
    ("主让深盘 ≥1.25", 1.25, None),
    ("主让 0.75~1.0", 0.75, 1.25),
    ("主让 0.25~0.5", 0.25, 0.75),
    ("平手附近", -0.25, 0.25),
    ("客让 0.25~0.5", -0.75, -0.25),
    ("客让 0.75~1.0", -1.25, -0.75),
    ("客让深盘 ≤-1.25", None, -1.25),
]
# 风险桶提示文案（写死可配置）
ODDS_BUCKET_RISK_NOTES = {
    "home": {
        (2.20, 2.80): "2.20–2.80 为假热门敏感区",
    },
    "fav": {
        (2.20, 2.80): "2.20–2.80 为假热门敏感区",
    },
...
# ── 稳胆策略（正确率优先，SP 甜区 1.3–1.6）────────────────
ACCURACY_SP_MIN = 1.30
...
# 比分区间预测
SCORE_RANGE_TOP_N = int(os.getenv("SCORE_RANGE_TOP_N", "5"))
SCORE_RANGE_EXACT_TOP_N = int(os.getenv("SCORE_RANGE_EXACT_TOP_N", "5"))
SCORE_RANGE_MIN_SAMPLES = int(os.getenv("SCORE_RANGE_MIN_SAMPLES", "5"))
SCORE_RANGE_LOW_MAX = int(os.getenv("SCORE_RANGE_LOW_MAX", "1"))
SCORE_RANGE_MID_MAX = int(os.getenv("SCORE_RANGE_MID_MAX", "3"))
