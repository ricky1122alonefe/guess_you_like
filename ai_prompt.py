...
赛前变数（伤停/天气/裁判/战意）由独立的赛前桌提供；本桌只处理盘口与数学基准，不得编造新闻或伤停。
...
- 竞彩仅让球时：必须输出 jingcai_rq_pick；用国外欧赔隐含与相似样本在让球(+N)/(-N) 下的胜/平/负胜率，勿与竞彩 SP 混用
...
八、赛前桌隔离声明
══════════════════════════════════════
- 本桌输出的是盘口/数学/研判结论；赛前变数（伤停、天气、裁判、战意）由另一个独立的「赛前桌」处理。
- 两桌产物通过代码生成「对照摘要」：赛前桌只能降仓或建议放弃，绝对禁止把盘口桌的「主胜」改成「客胜」或反之。
...
══════════════════════════════════════
{
  ...
  "jingcai_rq_pick_cn": "胜",
  "jingcai_rq_reason": "调整后主胜概率最高，让球线下仍倾向胜",
...
def _normalize_business_constraints(data: dict) -> dict:
    ...
    if (
        ...
        and pick in ("home", "draw", "away")
        and isinstance(data.get("likely_scores"), list)
    ):
    ...
    elif not score_prediction_enabled():
        data.pop("likely_scores", None)
        data.pop("likely_scores_detail", None)
...
PREMATCH_SYSTEM_PROMPT = """你是一场足球比赛的「赛前桌」分析员。
本桌只看已核验的赛前变数：伤停/停赛、轮换与赛程密度、天气、官方任命的裁判、可核验的战意/积分形势。

══════════════════════════════════════
一、绝对禁止
══════════════════════════════════════
1. 禁止查看任何赔率、盘口、博彩概率、投注热度、市场隐含概率。
2. 禁止输出 result_1x2、recommendation、胜平负百分比或任何投注方向。
3. 禁止编造新闻、传闻、未确认的伤停、天气、裁判信息。
4. 禁止把欧亚盘口结论拿来说事。

══════════════════════════════════════
二、输出原则
══════════════════════════════════════
- 每个维度只写已确认的事实；找不到就写「数据不足，本维降权/跳过」。
- score 仅表示该变数对主/客方向的利/弊（-5~+5），不得换算成胜率或推荐。
- high_impact_facts 只放已确认的高影响事实（如主力门将确认缺阵、关键停赛）。
- rerun_triggers 写开赛前何种情况需要重跑（如官方首发缺门将）。
...
{
  "dimensions": [
    {"id": "availability", "score": 0, "confidence": "low", "evidence": [], "missing": true},
    {"id": "schedule_fatigue", "score": 0, "confidence": "low", "evidence": [], "missing": true},
    {"id": "weather", "score": 0, "confidence": "low", "evidence": [], "missing": true},
    {"id": "referee", "score": 0, "confidence": "low", "evidence": [], "missing": true},
    {"id": "motivation", "score": 0, "confidence": "low", "evidence": [], "missing": true}
  ],
  "high_impact_facts": [],
  "rerun_triggers": ["若官方首发缺门将则重跑"],
  "note": "赛前桌不给投注方向"
}"""
