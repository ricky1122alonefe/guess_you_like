...
from jingcai_pick import actionable_jingcai_pick, ensure_match_jingcai, final_recommendation_cn, resolve_jingcai_sp
...
2. 组合回报按 candidates 中的 jingcai_sp（国内竞彩 SP）相乘，不要用 eu_odds 欧赔。
3. 不要只看单场胜率，要考虑两场组合风险叠加。
4. 初盘支持但实时盘变弱、欧赔挺但亚盘不跟、平赔下调、诱盘/控盘/过热，均要降级。
5. 每组两场必须来自同一个 match_date；用户提供的候选已经按同一天过滤，禁止跨天组合。
6. safe 与 yield 尽量不要完全相同；候选不足时可重叠，但 reason 必须说明。
7. 如果候选整体质量差，也必须选相对最合理的两组，但 reason 要明确“小仓位/不重仓”。
8. 禁止编造候选外比赛、赔率、伤停或新闻。
...
def _candidate(m: dict, kickoff_map: dict | None) -> dict[str, Any] | None:
    ...
    if not pick_key or pick_key == "skip":
        return None
    market = (jc or {}).get("market") or (m.get("jingcai_pick_info") or {}).get("jingcai_market")
    ...
    *,
    ...
    for ai in ai_options:
        ...
        if len(local.get("legs") or []) == 2:
            combined = _combined_odds(local["legs"])
